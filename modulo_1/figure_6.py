...
def perform_fit(model, x, y, yerr, p0):
    popt, pcov = curve_fit(parabola, x, y, sigma=yerr, p0=p0, absolute_sigma=True)
...
for model in modelli:
    popt_B, pcov_B = curve_fit(power_law, L_array, fit_params[model]['B'], sigma=fit_params[model]['B_err'], p0=[1, -1, 0], absolute_sigma=True)
    fit_params_power_B[model] = popt_B
    fit_errors_power_B[model] = np.sqrt(np.diag(pcov_B))

    popt_C, pcov_C = curve_fit(power_law, L_array, fit_params[model]['C'], sigma=fit_params[model]['C_err'], p0=[1, -1, 0], absolute_sigma=True)
...
for i, model in enumerate(modelli):
    ...
    axes[0].plot(L_dense, fit_B, color=color, linewidth=params.get('line_width_axes', 1), alpha=0.5)
...
for i, model in enumerate(modelli):
    ...
    axes[2].plot(L_dense, fit_C, color=color, linewidth=params.get('line_width_axes', 1), alpha=0.5)
