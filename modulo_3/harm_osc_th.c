...
double correlator(double const * const restrict lattice, long int Nt, long int deltat, int n){
  long int r, raux;
  int i;
  double res = 0;
  ...
  for(r=0; r<Nt; r++){
    ...
    res += pow(lattice[r], n) * pow(lattice[raux], n);
  }
...
int montecarlo(int Nt, double simbeta, long int sample){
  ...
  for(int iter = 0; iter < sample; iter++){
    ...
    if(iter%measevery==0){
      ...
      fprintf(fp, "%.12f %.12f %.12f ", x, x2, Knaive);

      for (int n = 1; n < 2; n++){ 
        for(int deltat=0; deltat < Nt/4; deltat++){
              
          fprintf(fp, "%.12f ", correlator(lattice, Nt, deltat, n));
        }
      }
